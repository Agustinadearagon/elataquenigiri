...
function init() {
    ...
    caraImg.onload = () => console.log("Cara cargada", caraImg.naturalWidth, caraImg.naturalHeight);
    caraImg.onerror = () => console.error("Error cargando cara.jpg");
    caraImg.src = "cara.jpg?v=2";
    resize();
...
function dibujarCara() {
    const cx = player.x + FACE_SIZE / 2;
    const cy = player.y + FACE_SIZE / 2;
    const r = FACE_SIZE / 2;
    ...
    if (caraImg.complete && caraImg.naturalWidth > 0) {
        // Cover: llenar el círculo centrando la cara
        const iw = caraImg.naturalWidth;
        const ih = caraImg.naturalHeight;
        const scale = Math.max(FACE_SIZE / iw, FACE_SIZE / ih);
        const dw = iw * scale;
        const dh = ih * scale;
        const dx = player.x + (FACE_SIZE - dw) / 2;
        const dy = player.y + (FACE_SIZE - dh) / 2;
        ctx.drawImage(caraImg, dx, dy, dw, dh);
    } else {
